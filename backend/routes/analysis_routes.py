...
async def preliminary_analysis():
    ...
    try:
        analysis_result = await analyze_content(content_type, content)
...
async def cross_verification():
    ...
    try:
        result = await cross_verify_content(data['content'], data['analysis'])
...
async def context_analysis():
    ...
    try:
        result = await analyze_context(data['content'])
        return jsonify(result)
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@analysis_bp.route('/final', methods=['POST'])
async def final_evaluation_route():
    ...
    try:
        result = await final_evaluation(data['user_perception'], data['ai_analysis'])
        return jsonify(result)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
